...
import { updateScheduleItem, getScheduleById } from "@/app/actions/schedule";
...
export default function EditSchedulePage() {
  ...
  useEffect(() => {
    getScheduleById(id).then((item) => {
      if (!item) { router.push("/admin/schedule"); return; }
      setForm({ time: item.time, event: item.event, displayOrder: item.displayOrder ?? 0 });
      setIsLoading(false);
    });
  }, [id, router]);
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      await updateScheduleItem(id, form);
  ...
  return (
    <div className="space-y-6 max-w-2xl mx-auto">
      <div className="flex items-center gap-4">
        ...
        <div>
          <h1 className="text-3xl font-bold tracking-tight text-white">Edit Schedule Item</h1>
          <p className="text-muted-foreground mt-2">Update timeline details.</p>
        </div>
      </div>
